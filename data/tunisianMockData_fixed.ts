...
const generateAppointmentData = (count: number = 25): MockAppointment[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    appointmentDate.setDate(appointmentDate.getDate() + Math.floor(Math.random() * 270));
...
export const generateMockMessages = (conversationId: string | number, count: number = 20): MockMessage[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    messageDate.setHours(messageDate.getHours() - Math.floor(Math.random() * 72));
...
// Generate mock appointment function (duplicate for backward compatibility)
export const generateMockAppointments = (patientId?: string) => {
  if (patientId) {
