// ==================================================================================
// Tunisian Mental Health App - IMMUTABLE MOCK DATA
// ==================================================================================
// This file contains comprehensive mock data for the MindCare AI app
// Includes realistic Tunisian names, addresses, and cultural context
//
// ⚠️  IMPORTANT: ALL DATA IN THIS FILE IS FROZEN AND IMMUTABLE ⚠️
// 
// This mock data is designed to be unchangeable at runtime to ensure:
// - Consistent app behavior across all sessions
// - No accidental mutations during development or testing
// - Reliable UI state testing with predictable data
// - Prevention of data corruption during app usage
//
// Do NOT attempt to modify this data at runtime. All exported objects and
// arrays are deeply frozen using Object.freeze().
// ==================================================================================
...
// Helper function to get random placeholder image - MUST be defined after PLACEHOLDER_IMAGES
// ⚠️  WARNING: This function is for data generation only, not for runtime use
export const getRandomPlaceholderImage = () => {
  return PLACEHOLDER_IMAGES[Math.floor(Math.random() * PLACEHOLDER_IMAGES.length)];
};

// Safe alternative for runtime use - returns a deterministic image based on index
export const getPlaceholderImageByIndex = (index: number) => {
  return PLACEHOLDER_IMAGES[index % PLACEHOLDER_IMAGES.length];
};

// Safe alternative for runtime use - returns the default placeholder image
export const getDefaultPlaceholderImage = () => {
...
// Utility functions
// ⚠️  WARNING: These functions are for initial data generation only, not for runtime use
...
// Generate main data collections
...
const generateAppointmentData = (count: number = 25): MockAppointment[] => {
  ...
  // Create upcoming appointments specifically for Aziz Bahloul (so they show in the UI)
  const azizUpcomingCount = 3; // Ensure Aziz has at least 3 upcoming appointments
  for (let i = 0; i < azizUpcomingCount; i++) {
    const appointmentDate = new Date(now);
    appointmentDate.setDate(appointmentDate.getDate() + (i + 1) * 3 + Math.floor(Math.random() * 5)); // Spread them out
    ...
    appointments.push({
      id: `appointment_aziz_upcoming_${i + 1}`,
      patient: AZIZ_BAHLOUL, // Use Aziz specifically
      therapist: randomTherapist,
      date: appointmentDate.toISOString().split('T')[0],
      time,
      duration: [30, 45, 60, 90][Math.floor(Math.random() * 4)],
      status: Math.random() > 0.5 ? 'confirmed' : 'pending',
      notes: 'Upcoming therapy session for Aziz.',
      appointment_type: randomType,
      video_session_link: randomType === 'video' ? 'https://meet.mindcare.tn/session-aziz-' + (i + 1) : undefined,
      created_at: new Date(now.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      updated_at: new Date().toISOString()
    });
  }
  
  // Create some more upcoming appointments for other patients
  const otherUpcomingCount = Math.floor((count - azizUpcomingCount) * 0.3); // 30% of remaining
  for (let i = 0; i < otherUpcomingCount; i++) {
    const appointmentDate = new Date(now);
    appointmentDate.setDate(appointmentDate.getDate() + Math.floor(Math.random() * 30) + 1); // Next 30 days
    ...
    appointments.push({
      id: `appointment_other_upcoming_${i + 1}`,
      ...
      appointment_type: randomType,
      video_session_link: randomType === 'video' ? 'https://meet.mindcare.tn/session-other-' + (i + 1) : undefined,
      created_at: new Date(now.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      updated_at: new Date().toISOString()
    });
  }
  ...
  const pastCount = count - azizUpcomingCount - otherUpcomingCount;
  const startDate = new Date(now);
  startDate.setMonth(startDate.getMonth() - 6);
  
  // Ensure Aziz has some past appointments too
  const azizPastCount = Math.floor(pastCount * 0.4); // 40% for Aziz
  for (let i = 0; i < azizPastCount; i++) {
    const appointmentDate = new Date(startDate);
    appointmentDate.setDate(appointmentDate.getDate() + Math.floor(Math.random() * 180)); // Past 6 months
    ...
    appointments.push({
      ...
      notes: randomStatus === 'completed' ? 
        ...
        'Regular therapy session for Aziz.',
      appointment_type: randomType,
      video_session_link: randomType === 'video' ? 'https://meet.mindcare.tn/session-aziz-past-' + (i + 1) : undefined,
      created_at: new Date(appointmentDate.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      updated_at: new Date().toISOString()
    });
  }
  
  // Create past appointments for other patients
  for (let i = azizPastCount; i < pastCount; i++) {
    const appointmentDate = new Date(startDate);
    appointmentDate.setDate(appointmentDate.getDate() + Math.floor(Math.random() * 180)); // Past 6 months
    ...
    appointments.push({
      id: `appointment_other_past_${i + 1}`,
      patient: randomPatient,
      therapist: randomTherapist,
      date: appointmentDate.toISOString().split('T')[0],
      time,
      ...
      appointment_type: randomType,
      video_session_link: randomType === 'video' ? 'https://meet.mindcare.tn/session-other-past-' + (i + 1) : undefined,
      created_at: new Date(appointmentDate.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      updated_at: new Date().toISOString()
    });
  }
...
export const MOCK_APPOINTMENTS = generateAppointmentData(30);

// Generate posts with comments
const generatePostData = (count: number = 20): MockPost[] => {
...
export const MOCK_POSTS = generatePostData(25);

// Generate conversations
const generateConversationData = (count: number = 15): MockConversation[] => {
...
export const MOCK_CONVERSATIONS = generateConversationData(20);

// Generate comprehensive appointment data for all UI states
const generateMockAppointmentData = () => {
  return {
    upcoming: MOCK_APPOINTMENTS.filter(apt => {
    ...
    past: MOCK_APPOINTMENTS.filter(apt => {
      const aptDate = new Date(apt.date);
      const now = new Date();
    ...
    cancelled: MOCK_APPOINTMENTS.filter(apt => apt.status === 'cancelled'),
...
export const MOCK_NOTIFICATIONS: MockNotification[] = Array.from({ length: 15 }, (_, i) => {
  ...
  notificationDate.setHours(notificationDate.getHours() - Math.floor(Math.random() * 168));
...
// Export helper functions - Safe for runtime use
...
export const getAppointmentsByPatientId = (patientId: string) => 
  [...MOCK_APPOINTMENTS.filter((apt: MockAppointment) => apt.patient.id === patientId)];

export const getUpcomingAppointments = (patientId: string) => {
  const now = new Date();
  return [...MOCK_APPOINTMENTS.filter((apt: MockAppointment) => 
...
// Generate mock messages for conversations - Safe for runtime use
export const generateMockMessages = (conversationId: string | number, count: number = 20): MockMessage[] => {
  const messages: MockMessage[] = [];
  const conversation = MOCK_CONVERSATIONS.find((c: MockConversation) => c.id === conversationId);
  const participants = conversation?.participants || [AZIZ_BAHLOUL, SLIMEN_ABYADH];
  
  const sampleMessages = [
    'How are you feeling today?',
    'I practiced the breathing exercises you taught me.',
    'Thank you for the session yesterday, it was very helpful.',
    'I have a question about the homework you assigned.',
    'Looking forward to our next appointment.',
    'The mindfulness techniques are really working for me.',
    'I\'d like to discuss some challenges I\'ve been facing.',
    'Can we schedule a follow-up session?',
    'I feel much better after implementing your suggestions.',
    'Could you recommend any additional resources?'
  ];
  ...
  for (let i = 0; i < count; i++) {
    ...
    messageDate.setHours(messageDate.getHours() - (i * 3)); // Deterministic time spacing
    
    messages.push({
      id: `message_${conversationId}_${i + 1}`,
      content: sampleMessages[i % sampleMessages.length], // Deterministic content selection
...
// Generate mock appointment function (safe for runtime use)
export const generateMockAppointments = (patientId?: string): MockAppointment[] => {
  if (patientId) {
    return [...MOCK_APPOINTMENTS.filter((apt: MockAppointment) => apt.patient.id === patientId)];
  }
  return [...MOCK_APPOINTMENTS]; // Return copy to prevent mutations
};

// ==================================================================================
// SAFE DATA ACCESS FUNCTIONS
// ==================================================================================
// These functions provide safe access to mock data without risk of mutation

// Get copies of main data collections to prevent mutations
export const getAllPatients = (): MockPatientProfile[] => [...MOCK_PATIENTS];
export const getAllTherapists = (): MockTherapistProfile[] => [...MOCK_THERAPISTS];
export const getAllUsers = (): MockUser[] => [...MOCK_USERS];
export const getAllAppointments = (): MockAppointment[] => [...MOCK_APPOINTMENTS];
export const getAllPosts = (): MockPost[] => [...MOCK_POSTS];
export const getAllConversations = (): MockConversation[] => [...MOCK_CONVERSATIONS];
export const getAllNotifications = (): MockNotification[] => [...MOCK_NOTIFICATIONS];

// Get specific user data (main characters)
export const getAzizBahloul = (): MockPatientProfile => ({ ...AZIZ_BAHLOUL });
export const getSlimenAbyadh = (): MockTherapistProfile => ({ ...SLIMEN_ABYADH });

// Validate that mock data is immutable (for testing purposes)
export const validateMockDataImmutability = (): boolean => {
  try {
    // Try to modify the data - should fail silently or throw in strict mode
    (MOCK_PATIENTS as any).push({ test: 'data' });
    AZIZ_BAHLOUL.first_name = 'Modified';
    MOCK_APPOINTMENTS[0].status = 'modified' as any;
    
    // If we reach here in non-strict mode, the data might not be properly frozen
    console.warn('⚠️  Mock data may not be properly frozen');
    return false;
  } catch (error) {
    // Expected behavior - modifications should fail
    console.log('✅ Mock data is properly frozen and immutable');
    return true;
  }
};
...
export const getMockDataSummary = () => {
  return {
    ...
    totalTherapists: MOCK_THERAPISTS.length,
    ...
    upcomingAppointments: MOCK_APPOINTMENTS.filter(apt => 
      new Date(apt.date) >= new Date() && ['pending', 'confirmed'].includes(apt.status)
    ).length,
    completedAppointments: MOCK_APPOINTMENTS.filter(apt => apt.status === 'completed').length,
    mainPatient: AZIZ_BAHLOUL.full_name,
    mainTherapist: SLIMEN_ABYADH.full_name,
    isDataFrozen: Object.isFrozen(MOCK_PATIENTS) && Object.isFrozen(MOCK_APPOINTMENTS)
  };
};
...
// ==================================================================================
// Deep freeze utility function to make objects and arrays completely immutable
function deepFreeze<T>(obj: T): T {
  // Don't freeze primitives
...
// Apply deep freeze to all exported mock data arrays and objects
// This prevents any runtime modifications to the mock data
deepFreeze(TUNISIAN_FIRST_NAMES);
deepFreeze(TUNISIAN_LAST_NAMES);
deepFreeze(TUNISIAN_CITIES);
deepFreeze(TUNISIAN_STREETS);
deepFreeze(PLACEHOLDER_IMAGES);
deepFreeze(MENTAL_HEALTH_IMAGES);
deepFreeze(VIDEO_THUMBNAILS);
deepFreeze(SAMPLE_VIDEOS);

// Freeze individual user objects
deepFreeze(AZIZ_BAHLOUL);
deepFreeze(SLIMEN_ABYADH);

// Freeze main data collections
deepFreeze(MOCK_PATIENTS);
deepFreeze(MOCK_THERAPISTS);
deepFreeze(MOCK_USERS);
deepFreeze(MOCK_APPOINTMENTS);
deepFreeze(MOCK_POSTS);
deepFreeze(MOCK_CONVERSATIONS);
deepFreeze(MOCK_APPOINTMENT_DATA);
deepFreeze(MOCK_WAITING_LIST);
deepFreeze(MOCK_FEEDBACK_DATA);
deepFreeze(MOCK_RESCHEDULE_DATA);
deepFreeze(MOCK_APPOINTMENT_STATS);
deepFreeze(MOCK_NOTIFICATIONS);

// Freeze the default export object
deepFreeze(exports.default);

// Log immutability status in development
if (process.env.NODE_ENV === 'development') {
  console.log('🔒 Mock data has been frozen and is now immutable');
  console.log('📊 Mock data summary:');
  console.log(`  - ${MOCK_PATIENTS.length} patients`);
  console.log(`  - ${MOCK_THERAPISTS.length} therapists`);
  console.log(`  - ${MOCK_APPOINTMENTS.length} appointments`);
  ...
  console.log('✅ All data structures are deeply frozen and unchangeable');
}
