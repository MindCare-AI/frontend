...
export const updateTherapistAvailability = async (
  ...
  try {
    ...
    const profile = await validateTherapistPermissions();
    ...
    console.log(`🔍 Using therapist profile ID: ${profile.id}`);
    const endpoint = `${API_URL}/therapist/profiles/${profile.id}/availability/`;
